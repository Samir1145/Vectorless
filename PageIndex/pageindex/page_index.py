...
async def check_title_appearance(item, page_list, start_index=1, model=None):    
    ...
    prompt = load_prompt("check_title_appearance", title=title, page_text=page_text)
...
async def check_title_appearance_in_start(title, page_text, model=None, logger=None):    
    prompt = load_prompt("check_title_appearance_in_start", title=title, page_text=page_text)
    response = await llm_acompletion(model=model, prompt=prompt)
    response = extract_json(response)
    if logger:
...
def toc_detector_single_page(content, model=None):
    prompt = load_prompt("toc_detector_single_page", content=content)
...
def check_if_toc_extraction_is_complete(content, toc, model=None):
    prompt = load_prompt("check_toc_extraction_complete", content=content, toc=toc)
    response = llm_completion(model=model, prompt=prompt)
    json_content = extract_json(response)
...
def check_if_toc_transformation_is_complete(content, toc, model=None):
    prompt = load_prompt("check_toc_transformation_complete", content=content, toc=toc)
    response = llm_completion(model=model, prompt=prompt)
    json_content = extract_json(response)
    return json_content['completed']

def extract_toc_content(content, model=None):
    prompt = load_prompt("extract_toc_content", content=content)
...
def detect_page_index(toc_content, model=None):
    ...
    prompt = load_prompt("detect_page_index", toc_content=toc_content)
    response = llm_completion(model=model, prompt=prompt)
    json_content = extract_json(response)
...
def toc_index_extractor(toc, content, model=None):
    ...
    prompt = load_prompt("toc_index_extractor", toc=str(toc), content=content)
    response = llm_completion(model=model, prompt=prompt)
    json_content = extract_json(response)    
    return json_content
...
def toc_transformer(toc_content, model=None):
    ...
    prompt = load_prompt("toc_transformer", toc_content=toc_content)
...
def add_page_number_to_toc(part, structure, model=None):
    prompt = load_prompt("add_page_number_to_toc", part=part, structure=json.dumps(structure, indent=2))
...
def generate_toc_continue(toc_content, part, model=None):
    ...
    prompt = load_prompt("generate_toc_continue", part=part, toc_content=json.dumps(toc_content, indent=2))
    response, finish_reason = llm_completion(model=model, prompt=prompt, return_finish_reason=True)
...
def generate_toc_init(part, model=None):
    ...
    prompt = load_prompt("generate_toc_init", part=part)
    response, finish_reason = llm_completion(model=model, prompt=prompt, return_finish_reason=True)
